...
// aes-js exports its CTR class lowercase; alias it so `new` satisfies new-cap.
const AesCtr = aesjs.ModeOfOperation.ctr
...
export const LargeSecureStore = {
  async getItem(key: string): Promise<string | null> {
    ...
    try {
      ...
      const cipher = new AesCtr(keyBytes, new aesjs.Counter(fromHex(counterHex)))
  ...
  async setItem(key: string, value: string): Promise<void> {
    ...
    const cipher = new AesCtr(keyBytes, new aesjs.Counter(counterBytes))
